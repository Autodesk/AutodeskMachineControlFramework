...
class Connection {
 public:
	...
	RestClient::Response postbinary(const std::string& uri,
		const unsigned char * pData, const unsigned int size);
	RestClient::Response put(const std::string& uri,
                             const std::string& data);
	RestClient::Response putbinary(const std::string& uri,
		const unsigned char * pData, const unsigned int size);
	RestClient::Response del(const std::string& uri);
	RestClient::Response patch(const std::string& uri, const std::string &data);

	void setProgressCallback(ConnectionProgressCallback pCallback, void * pUserData);
	ConnectionProgressCallback getProgressCallback ();
	void * getProgressUserData ();
 ...
 private:
	...
	ConnectionProgressCallback m_pProgressCallback;
	void * m_pProgressUserData;
};
};  // namespace RestClient
