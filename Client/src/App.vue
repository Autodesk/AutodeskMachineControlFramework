<template>
	<v-app id="inspire">
				...
				<template v-for="item in AppDefinition.MenuItems">
					<v-list-group
						...
						<v-list-item
								...
								:key="i"
								link
								v-on:click.stop="uiChangePage (child.targetPage)"
								>
					...
					<v-list-item
							...
							:key="item.caption"
							link
							v-on:click.stop="uiChangePage (item.targetpage)"
							>
						<v-list-item-action>
				...
				<v-row align="center" justify="center">
					<v-col cols="12" sm="8" md="4">
						<v-card class="elevation-12">
							...
							<v-card-actions>
								<v-spacer />
								<v-btn color="primary" v-on:click="uiReloadPageClick"><v-icon left>mdi-refresh</v-icon>Reload page</v-btn>
							</v-card-actions>
						</v-card>
					</v-col>
				</v-row>
		...
		<v-footer
				...
				>
			<v-spacer/>
			<span class="caption white--text">&copy; {{ AppDefinition.TextCopyRight }}</span>
		</v-footer>	
  ...
  export default {
	...
	methods: {
		...
		appRetrieveConfiguration () {
			...
			Axios({ method: "GET", "url": url }).then(resultJSON => {			
