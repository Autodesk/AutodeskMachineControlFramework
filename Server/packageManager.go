...
func main() {
	...
	if (len (argsWithProg) < 5) {
		log.Fatal ("Please start with packageManager <repositoryurl> <localfilename> <githash> <sha256checksum>");
	}
				
	downloadURLBase := argsWithProg[1];
	localFileName := argsWithProg[2];
	gitHash := argsWithProg[3];
	shaCheckSum := argsWithProg[4]; 
